...
I0pump=np.linspace(Imin*1e9,Imax*1e9,51*2) # W/m2 

Ipumpf= (gpump/2)**2 / ( (ws-wpump)**2 + (gpump/2)**2 )


#following illy reference
#20 kW/cm2*1.5ns=  30 uJ/mm2 
...
#dipole sums, from file
file1=np.genfromtxt(gfrom)#, np.vstack([k0s*L,np.real(Gxx),np.imag(Gxx)]).T)
#file2=np.genfromtxt('Gyy-small-40400.txt')#, np.vstack([k0s*L,np.real(Gyy),np.imag(Gyy)]).T)

print(nh*kmin*L,nh*kmax*L)


maskGxx= np.logical_and(file1[:,0]>nh*kmin*L ,file1[:,0]<nh*kmax*L  ) 


dataGxx= np.vstack([file1[maskGxx,0] , file1[maskGxx,1],file1[maskGxx,2]])
...
if False:
	...
	def err( ab ):
		...
		return np.sum(np.abs( powlaw( a, b, c) - file1[maskGxx,1]  )**2  )

	def err2( ab ):
		a,b,c=ab[0], ab[1], ab[2]
		return np.sum(np.abs( powlaw( a, b, c) - file1[maskGxx,2]  )**2  )


	from scipy.optimize import minimize
	x0r=[np.mean(file1[maskGxx,1]),1.2,np.min(file1[maskGxx,1])]
	x0i=[np.mean(file1[maskGxx,2]),1.2,np.min(file1[maskGxx,2])]
	x=file1[maskGxx,0]
	opt=minimize( err,  np.array(x0r)  , method='Nelder-Mead' )
	print('success', opt.success)
	opt2=minimize( err2,  np.array(x0i)  , method='Nelder-Mead' )
	print('success', opt.success)
	
	x= nh*k0s*L
	app3=powlaw( opt.x[0],  opt.x[1],  opt.x[2])
	app4=powlaw(opt2.x[0], opt2.x[1], opt2.x[2])

#	app3=powlaw(x0r[0], x0r[1], x0r[2])
#	app4=powlaw(x0i[0], x0i[1], x0i[2])
	
	Gxx=app3+1j*app4
#	Gyy=app3+1j*app4  ## ojo q no se usa
	
else:
	from scipy.interpolate import interp1d
	funcGr=interp1d(dataGxx[:,0], dataGxx[:,1], kind='linear',fill_value='extrapolate') 
	funcGi=interp1d(dataGxx[:,0], dataGxx[:,2], kind='linear',fill_value='extrapolate') 
	
	
	x= nh*k0s*L
	Gxx=funcGr(x)+1j*funcGi(x)

Gyy=Gxx*0

fig, ax1 =plt.subplots(nrows=1, ncols=1, sharex=True, figsize=(3.5, 2.3), dpi=200)

ax1.plot( dataGxx[:,0],dataGxx[:,1],label='fileR')
ax1.plot( dataGxx[:,0],dataGxx[:,2],label='fileI')
ax1.plot( nh*k0s*L,np.real(Gxx),'--',label='optR')
ax1.plot( nh*k0s*L,np.imag(Gxx),'--',label='optI')
#ax1.set_yscale('log')
ax1.set_ylabel('$G$')
ax1.set_xlabel("KL")
#ax1.set_xlim(nh*kmin*L,nh*kmax*L)
#ax1.set_xlim(11,14)
ax1.set_ylim(np.min(dataGxx[:,1]),np.max(dataGxx[:,1]))
#ax1.legend(loc='upper left')
...
def polariz_QSshelli_lattice2(e0,e1,e2,r0,r1,Gxx,Gyy):		
	#G in 1/m
	alpha0 =r0**3*(e0-e1)/(e0+2*e1)	
	f0=(1-alpha0/r1**3)/(1+2*alpha0/r1**3)	
	alpha1=r1**3*(e1-f0*e2 ) /(e1+2*f0*e2)
	
	alphaSI=alpha1*4*np.pi*eps0		
	alpha_ef=( e2*eps0 / alphaSI - ks**2*Gxx  )**-1		
...
#		Zp= np.abs(E[:,:,175])		
		...
		#Zp= Zp/np.max(E)
		fig, ax1 =plt.subplots(nrows=1, ncols=1, sharex=True, figsize=(1.215, 1.9), dpi=200)
#		norm = colors.LogNorm()		(1.625, 1.9)
		norm = colors.Normalize()#vmin=0,vmax=1)	
		c = ax1.pcolormesh(Xp,Yp , Zp**.5, cmap=cmap2,norm=norm, rasterized=True)	
		
		cbar=fig.colorbar(c, ax=ax1,fraction=0.046, pad=0.0, orientation='horizontal', location='top' ) #,ticks=[2,4,6])
		...
		ax1.tick_params( direction='in', which='both')
		#ax1.text(1.35, -0.15, '(a)', transform=ax1.transAxes,
...
xx=polariz_QSshelli_lattice2(epsinic,epsAg,epsm,r0,r1,Gxx,Gyy)
...
alphaCS=polariz_QSshelli_lattice2(epsinic,epsAg,epsm,r0,r1,Gxx,Gyy)

r=1j*ks/2/S_L*(alphaCS)
R=np.abs(r)**2	
t=1j*ks/2/S_L*(alphaCS)+1
T=np.abs(t)**2	
A=1-T-R
Trans=1-T
...
#	N2i=rates(Isource)		
	...
	alphai=polariz_QSshelli_lattice2(epsi,epsAg,epsm,r0,r1,Gxx,Gyy)
	...
	while converg>1e-6:
		...
		counter+=1
		if counter>60: converg=0
			
		
	print(counter)
	alphaCSi=polariz_QSshelli_lattice2(epsi,epsAg,epsm,r0,r1,Gxx,Gyy)

	r=1j*ks/2/S_L*(alphaCSi)
	R=np.abs(r)**2	
	t=1j*ks/2/S_L*(alphaCSi)+1
	T=np.abs(t)**2	
	A=1-T-R
...
if True:
	...
	ax1.set_xlabel('$I_{p}$ ($ kW/cm^2 $)')
	ax1.set_ylabel("$N_2$")		
	ax2.set_ylabel("Purcell factor")		
	
	ax1.tick_params( direction='in', which='both')
	ax2.tick_params( direction='in', which='both')
	plt.axvline(unst_min, ls='--', c='chocolate', lw=0.75)
	plt.axvline(unst_max, ls='--', c='chocolate', lw=0.75)
...
Z=np.imag(eps**.5).T
linthresh=np.quantile(np.abs(Z) , 0.75)  
...
cbar=fig.colorbar(c, ax=ax1,ticks=[1000, 100,10,1], orientation='horizontal', location='top' )
cbar.ax.tick_params(axis='x', direction='in', which ='both')
...
ax1.tick_params( direction='in', which='both')
ax2.tick_params( direction='in', which='both')
#ax1.text(1.3, -0.12, '(e)', transform=ax1.transAxes,
      #fontsize=8, fontweight='bold', va='top', ha='right')
if False:
...
#ax1.text(1.4, -0.13, '(c)', transform=ax1.transAxes,
      #fontsize=8, fontweight='bold', va='top', ha='right')
...
#labels=[ "$\lambda_{pump}$" , "$\lambda_{abs}$" , "$\lambda_{ems}$" ]
##secax_x.set_xticklabels(labels)
cbar.set_label(r'$\bf{Re\{ \alpha \}}$ ($F \mu m^2$)',labelpad=5)
...
#ax1.text(1.4, -0.13, '(d)', transform=ax1.transAxes,
      #fontsize=8, fontweight='bold', va='top', ha='right')
...
cbar.set_label(r'$\bf{Im\{ \alpha \}}$ ($F \mu m^2$)',labelpad=5)
ax1.set_ylabel(r'$\bf{  I_{p}}$ ($kW/mm^2 $)',labelpad=0)
ax1.set_yticks(Iticks, minor=False)
ax1.set_yticks(Imticks, minor=True)
ax1.set_xticks(wlticks, minor=False)
